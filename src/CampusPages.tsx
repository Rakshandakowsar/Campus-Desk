...
function AIPage({ studentId }: { studentId: string }) {
  ...
  async function ask(event: FormEvent<HTMLFormElement>) {
    ...
    setIsSending(true)
    try {
      const response = await fetch(apiUrl('/api/chat'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ question, student_id: studentId }) })
...
function FeeReceiptControls({ onOpenAction }: { onOpenAction: (action: string) => void }) {
  const [receiptOpen, setReceiptOpen] = useState(false)

  return <>
    <div className="action-row">
      <button className="secondary-button" onClick={() => setReceiptOpen((value) => !value)}><FileCheck2 size={15} /> {receiptOpen ? 'Hide receipt' : 'Receipts'}</button>
      <button className="primary-button" onClick={() => onOpenAction('support')}><Headphones size={15} /> Fee-related support</button>
    </div>
    {receiptOpen && <div className="feature-card" style={{ padding: '18px 20px', marginTop: '18px' }}>
      <p className="eyebrow">RECEIPT PREVIEW</p>
      <h3 style={{ fontSize: '18px', marginBottom: '12px' }}>Semester 7 Tuition Receipt</h3>
      <div style={{ display: 'grid', gap: '8px', fontSize: '12px', color: '#475b63' }}>
        <div><strong>Invoice ID:</strong> INV-2026-0827</div>
        <div><strong>Student:</strong> Arjun Sharma</div>
        <div><strong>Roll No:</strong> 23CS1042</div>
        <div><strong>Amount:</strong> Rs 15,000</div>
        <div><strong>Payment status:</strong> Pending</div>
        <div><strong>Issued on:</strong> 09 September 2026</div>
      </div>
    </div>}
  </>
}
...
function GenericFeaturePage({ page, studentId, onOpenAction, onNavigate }: CampusPageProps) {
  const [examReminder, setExamReminder] = useState<string | null>(null)
  const [timetableDownloadNotice, setTimetableDownloadNotice] = useState<string | null>(null)

  function handleExamReminder() {
    setExamReminder('Reminder: Your next exam is Deep Learning on 20 September 2026 at 10:00 AM in Room A-204. Please arrive 15 minutes early and carry your hall ticket.')
    setTimetableDownloadNotice(null)
  }

  function handleDownloadTimetable() {
    const timetableText = [
      'Campusdesk Exam Timetable',
      '------------------------',
      'Deep Learning | 20 September 2026 | 10:00 AM | Room A-204',
      'Machine Learning | 24 September 2026 | 2:00 PM | Room B-110',
      'NLP Lab Viva | 28 September 2026 | 11:30 AM | Innovation Hub',
    ...
    link.href = url
    link.download = 'campusdesk-exam-timetable.txt'
    document.body.appendChild(link)
    link.click()
    link.remove()
    URL.revokeObjectURL(url)

    setTimetableDownloadNotice('Exam timetable downloaded successfully.')
  ...
  if (page === 'Exams') return <><PageHeader page={page} /><div className="metric-grid"><InfoCard icon={CalendarDays} title="Upcoming exams" value="3" detail="Demo timetable records" /><InfoCard icon={Clock3} title="Next exam" value="20 Sep" detail="Deep Learning · 10:00 AM" /></div><FeatureSection title="Upcoming exams" eyebrow="DEMO TIMETABLE"><div className="data-grid">{demoCards.Exams.map(([subject, date, time, room]) => <article className="feature-card exam-card" key={subject}><CalendarDays size={18} /><div><h3>{subject}</h3><p>{date} · {time}</p><span>{room} · End Semester Examination</span></div><BadgeCheck size={17} className="good-icon" /></article>)}</div></FeatureSection>{examReminder && <div className="demo-note"><Info size={15} /> {examReminder}</div>}{timetableDownloadNotice && <div className="demo-note"><CheckCircle2 size={15} /> {timetableDownloadNotice}</div>}<div className="action-row"><button className="secondary-button" onClick={handleExamReminder}><CalendarDays size={15} /> Exam reminders</button><button className="secondary-button" onClick={handleDownloadTimetable}><ArrowUpRight size={15} /> Download timetable</button></div></>
  if (page === 'Fees') return <><PageHeader page={page} /><div className="metric-grid"><InfoCard icon={WalletCards} title="Total fee" value="Rs 50,000" detail="Demo fee summary" tone="blue" /><InfoCard icon={CheckCircle2} title="Paid" value="Rs 35,000" detail="Demo payment history" tone="green" /><InfoCard icon={CircleAlert} title="Pending" value="Rs 15,000" detail="Due date: demo data" tone="coral" /></div><FeatureSection title="Payment history" eyebrow="DEMO DATA"><div className="table-list"><div><span>Semester 6 tuition</span><strong>Paid · Rs 20,000</strong></div><div><span>Semester 7 tuition</span><strong>Paid · Rs 15,000</strong></div><div><span>Pending semester fee</span><strong className="attention-text">Due date to be confirmed</strong></div></div></FeatureSection><FeeReceiptControls onOpenAction={onOpenAction} /></>
...
function AdminPage() {
  ...
  useState(() => { void fetch(apiUrl('/api/analytics')).then((response) => response.json()).then(setAnalytics).catch(() => setAnalytics(null)) })
